...
(async () => {
  ...
  for (const [face, yaw, px_, pz_] of [['+z', Math.PI, 0, 8], ['-z', 0, 0, -8], ['+x', -Math.PI / 2, 8, 0], ['-x', Math.PI / 2, -8, 0]]) {
    await ev(([yaw, dx, dz]) => { window.__debugBoxes.length = 0; W.state.time = 12; sim(0.2); tp(300 + dx, 336 + dz, yaw); clearArea(300, 336, 60); const rx = -Math.cos(yaw), rz = Math.sin(yaw); /* screen-right for this yaw */ const bx = 300 + rx * 3.2, bz = 336 + rz * 3.2; __debugBox(bx - 1.5, 0.3, bz - 1.5, 3, 3, 3, [1, 1, 1], TEX.names.testcard, 0); /* lit, not emissive: emissive adds the vertex colour and would wash the card white */ }, [yaw, px_, pz_]);
    await shot('face' + face);
    const mid = [Math.round(W * 0.25 * dpr), Math.round(H * 0.08 * dpr), Math.round(W * 0.95 * dpr), Math.round(H * 0.8 * dpr)];
