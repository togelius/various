...
(async () => {
  ...
  await page.evaluate(() => { GAME.startPlay(); MISSIONS.S.dialogue = null; MISSIONS.S.progress = 1; MISSIONS.S.cooldown = 1e9; window.__manual = true; window.__pt = { paused: true }; // the frame loop is parked; every render is explicit
    ...
    window.px = { load(b64) { return new Promise(res => { const img = new Image(); img.onload = () => { const c = document.createElement('canvas'); c.width = img.width; c.height = img.height; const g = c.getContext('2d'); g.drawImage(img, 0, 0); window.__shot = g.getImageData(0, 0, c.width, c.height); res([c.width, c.height]); }; img.src = 'data:image/png;base64,' + b64; }); },
      ...
      // Like find, but only inside a ring: the on-foot route line now shares the blip's colour, so the edge blip is
      // located among pixels near the radar rim.
      ring(pred, cx, cy, r0, r1) { const s = window.__shot; let n = 0, sx = 0, sy = 0; const d = s.data; for (let y = Math.max(0, Math.floor(cy - r1)); y < Math.min(s.height, cy + r1); y++) for (let x = Math.max(0, Math.floor(cx - r1)); x < Math.min(s.width, cx + r1); x++) { const q = Math.hypot(x - cx, y - cy); if (q < r0 || q > r1) continue; const i = (y * s.width + x) * 4; if (pred(d[i], d[i + 1], d[i + 2])) { n++; sx += x; sy += y; } } return n ? { n, x: sx / n, y: sy / n } : { n: 0, x: -1, y: -1 }; },
  ...
  { const ahead = await blipAt(0, -30, 'ahead'); check('radar: target ahead draws above the centre', ahead.n > 10 && ahead.y < (radarC.cy - 12) * dpr && Math.abs(ahead.x - radarC.cx * dpr) < 14 * dpr, { x: Math.round(ahead.x - radarC.cx * dpr), y: Math.round(ahead.y - radarC.cy * dpr), n: ahead.n });
    ...
    const ringAt = () => ev(([cx, cy, R]) => px.ring(COL.magenta, cx, cy, R - 16, R + 8), [radarC.cx * dpr, radarC.cy * dpr, radarC.R * dpr]);
    await blipAt(0, -400, 'edge'); const far = await ringAt(); check('radar: off-radar target ahead sits at the top edge', far.n > 10 && far.y < (radarC.cy - radarC.R + 20) * dpr && Math.abs(far.x - radarC.cx * dpr) < 14 * dpr, { x: Math.round(far.x - radarC.cx * dpr), y: Math.round(far.y - radarC.cy * dpr), n: far.n });
    ...
    const turned = await ringAt(); check('radar: after turning right the target swings to the left edge', turned.n > 10 && turned.x < (radarC.cx - radarC.R + 20) * dpr && Math.abs(turned.y - radarC.cy * dpr) < 14 * dpr, { x: Math.round(turned.x - radarC.cx * dpr), y: Math.round(turned.y - radarC.cy * dpr), n: turned.n });
