...
(async () => {
  ...
  const res = await p.evaluate(() => {
    ...
    try {
      ...
      // M7
      d = begin(7); note(7, 'start', { crew: d.crew.length }); const bank = CITY.place('bank'); if (P.car) PLAYER.exitCar(); P.car = null; P.state = 'foot'; P.x = bank.x; P.z = bank.z + 2; /* the heist goes in through the front door, on foot */ for (const c of d.crew) { c.x = bank.x + 2; c.z = bank.z + 3; } sim(2); note(7, 'at bank', { phase: d.phase, timer: Math.round(S.timer), wanted: PLAYER.wanted });
      ...
      // vigilante
      PLAYER.exitCar(); const vl = CITY.nearestLane(P.x, P.z), [vx, vz] = CITY.lanePoint(vl.e, 1, vl.s); for (const o of W.cars.slice()) if (!o.removed && M.dist(o.x, o.z, vx, vz) < 10) o.remove(); /* on open road, clear of wherever the taxi stopped */ const cop = VEH.spawn('police', vx, vz, Math.atan2(vl.e.dx, vl.e.dz), { mode: 'parked' }); const inCop0 = enter(cop); sim(0.1, ['KeyT']); sim(1); note('vig', 'started', { side: S.side && S.side.kind, target: !!(S.side && S.side.target), inCop: PLAYER.car === cop, inCop0, st: P.state, near: W.cars.filter(c => !c.removed && M.dist(c.x, c.z, P.x, P.z) < 8).map(c => c.type).join('+') }); if (S.side && S.side.target) { S.side.target.explode(); sim(1); note('vig', 'kill', { level: S.side && S.side.level }); }
