...
const INPUT = (() => {
  ...
  function init(c) {
    canvas = c;
    window.addEventListener('keydown', e => { if (!keys[e.code]) pressed[e.code] = true; keys[e.code] = true; if (e.key && e.key.length === 1) { api.typed = (api.typed + e.key.toUpperCase()).slice(-12); } if (['Space', 'Tab', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'KeyF'].includes(e.code) || e.code.startsWith('Digit')) e.preventDefault(); });
  ...
  const api = { init, down, hit, mouse, pad, pollPad, endFrame, requestLock, releaseLock, get locked() { return locked; }, onLockLost: null, typed: '' };
  return api;
})();
