...
const SETTINGS = (() => {
  const defaults = Object.freeze({ sensitivity: 1, aimSensitivity: .75, invertY: false,
    ...
    steeringAssist: .35, gripAssist: .6, hudScale: 1, perfOverlay: false, perfShare: false, touchScale: 1, touchOpacity: .85, touchLeft: false, bloom: true, resolution: 1.25, shadows: true,
  ...
  function sanitize(o) {
    for (const [key, min, max] of [['sensitivity',.3,3],['aimSensitivity',.2,1.5],['cameraShake',0,1],['speedFov',0,1],['steeringAssist',0,1],['gripAssist',0,1],['hudScale',.8,1.3],['touchScale',.75,1.4],['touchOpacity',.3,1],['resolution',.75,1.5],...['masterVolume','sfxVolume','vehicleVolume','ambientVolume','musicVolume'].map(k=>[k,0,1])]) {
      o[key] = Number.isFinite(o[key]) ? Math.min(max,Math.max(min,o[key])) : defaults[key];
    }
    for (const key of ['invertY','mouseAssist','controllerAssist','bloom','shadows','auto','edges','quietMix','perfOverlay','touchLeft','perfShare']) if (typeof o[key] !== 'boolean') o[key] = defaults[key];
  ...
  function build() {
    ...
    const groups = [
      ['Camera & comfort', [['sensitivity','Look sensitivity',.3,3,.1],['aimSensitivity','Aim sensitivity',.2,1.5,.05],['cameraShake','Camera shake',0,1,.05],['speedFov','Speed / sprint FOV effect',0,1,.1],['hudScale','HUD & text scale',.8,1.3,.05],['touchScale','Touch button size',.75,1.4,.05],['touchOpacity','Touch button opacity',.3,1,.05],['touchLeft','Touch: fire buttons on the left'],['invertY','Invert vertical look'],['mouseAssist','Mouse aim assistance'],['controllerAssist','Controller / touch aim assistance'],['steeringAssist','Countersteer assistance',0,1,.05],['gripAssist','Tyre grip assistance (less sliding)',0,1,.05]]],
      ['Picture & sound', [['resolution','Render scale',.75,1.5,.25],['shadows','Sun shadows'],['bloom','Bloom & tone mapping'],['edges','Illustrated outlines'],['auto','Adaptive quality'],['perfOverlay','Performance overlay'],['perfShare','Send performance numbers to Claude (once a minute)'],['muted','Mute all sound'],['masterVolume','Master volume',0,1,.05],['sfxVolume','Effects volume',0,1,.05],['vehicleVolume','Vehicle volume',0,1,.05],['ambientVolume','Street & weather volume',0,1,.05],['musicVolume','Radio volume',0,1,.05],['quietMix','Quiet dynamic range']]]
    ];
