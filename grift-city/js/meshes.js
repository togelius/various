...
const MESH = (() => {
  ...
  function pedMesh(look, lod = false) {
    ...
    for (const [sx, bone, shin] of [[0.11, 4, 7], [-0.11, 5, 8]]) {
      ...
      b.loft([trouserRing(-legH*.5,.079,.079),trouserRing(-.37,.085,.09),trouserRing(-.25,.096,.107),trouserRing(-.10,.108,.12),trouserRing(.03,.106,.116)],pants,cloth,bone);
      ball(sx, -legH * 0.5, 0.005, 0.074, pants, bone, 7, 3);
      b.loft([trouserRing(-legH+.08,.064,.070),trouserRing(-.72,.06,.066),trouserRing(-.62,.073,.078),trouserRing(-.51,.079,.081),trouserRing(-legH*.5,.079,.079)],pants,cloth,shin,{capEnd:true});
    ...
    // Shoe tongues, toe caps and stitching survive the normal chase-camera distance; fine laces are near-only.
    for(const [sx,leg,shin,foot] of [[.11,4,7,12],[-.11,5,8,13]]) {
      const shoeEdge=shoes.map(c=>c*1.4+.035), stitch=pants.map(c=>c*1.2+.018);
      b.roundedBox(sx-.049,-.76,-.036,.098,.045,.125,.016,shoeEdge,0,foot,{n:1});
      b.loft([Builder.rrect(sx,.01,.064,.071,.035,1,-.77,'y'),Builder.rrect(sx,.01,.064,.071,.035,1,-.746,'y')],pants.map(c=>c*.72),0,shin);
      if(!lod) {
        for(let k=0;k<3;k++) seam([sx-.033,-.718-k*.008,.022+k*.023],[sx+.033,-.718-k*.008,.022+k*.023],[.33,.30,.26],foot,.003);
        seam([sx-.062,-.802,.105],[sx+.062,-.802,.105],shoeEdge,foot,.002);
        seam([sx+Math.sign(sx)*.103,-.08,0],[sx+Math.sign(sx)*.083,-.39,0],stitch,leg);
        seam([sx+Math.sign(sx)*.079,-.45,0],[sx+Math.sign(sx)*.062,-.72,0],stitch,shin);
        for(const [y,z,w] of [[-.34,.09,.055],[-.52,.08,.05],[-.70,.071,.04]]) seam([sx-w,y,z],[sx+w,y+.016,z-.003],pants.map(c=>c*.85),y>-.425?leg:shin,.002);
      }
    }
    ...
    if(jacket) {
      ...
      for(const side of [-1,1]) {
        ...
        if(!lod) {
          // Pocket welt, zipper edge, epaulettes and gently broken-in elbow fabric.
          seam([side*.060,.14,.142],[side*.068,.39,.156],edge,0,.003);
          b.polyOut([[side*.087,.46,.149],[side*.171,.458,.136],[side*.171,.403,.148],[side*.087,.401,.154]],topDk,0,0,-.2);
          seam([side*.088,.461,.153],[side*.167,.46,.141],edge,0);
          b.sphere(side*.128,.444,.152, .003,.004,.003,[.62,.56,.40],{segs:4,rings:2,bone:0});
          seam([side*.080,.595,-.12],[side*.193,.558,-.126],edge,0,.003);
        }
      }
      b.polyOut([[-.060,.665,-.089],[.060,.665,-.089],[.044,.616,-.117],[-.044,.616,-.117]],collar,0,.4,0);
      // Distinct rear yoke and two expansion seams, visible while running away from the camera.
      b.polyOut([[-.213,.53,-.133],[0,.475,-.15],[.213,.53,-.133],[.164,.607,-.109],[-.164,.607,-.109]],top.map(c=>c*.83),0,.3,0);
    ...
    if(style===1) {
      const ring=(y,rx,rz,base=false)=>Array.from({length:SEG},(_,i)=>{const a=i/SEG*M.TAU,f=Math.sin(a);return [Math.cos(a)*rx,y+(base?f*.057:Math.max(0,f)*.004),-.014+f*rz];});
      b.loft([ring(.186,.137,.119,true),ring(.253,.137,.122),ring(.294,.113,.099),ring(.326,.071,.060),ring(.343,.014,.012)],hair,0,1,{capEnd:true});
      if(!lod) for(let k=0;k<5;k++) { const x=-.084+k*.035; b.tube([x,.267+(.084-x)*.12,.089],[x*.52-.012,.336-Math.abs(x)*.08,.036],.007,.005,hair.map(c=>c*1.25+.008),0,1,6); }
    } else if (style !== 5) { b.sphere(0, headR * 1.1, -0.012, hairR * 0.95, hairR * 1.1, hairR * 1.0, hair, { segs: SEG, rings: lod ? 2 : 4, lat0: style === 1 ? 0.2 : 0.3, lat1: 1, bone: 1 }); b.sphere(0, headR * 1.08, -0.012, hairR * 0.95, hairR * 1.1, hairR * 1.02, hair, { segs: 12, rings: 3, lat0: (longHair || style === 2) ? -0.55 : -0.05, lat1: 0.32, az0: Math.PI * 1.12, az1: Math.PI * 1.88, bone: 1 }); } // cap and the back of the head
    ...
    if (!lod) {
      ...
      b.polyOut([[-.029,.091,.111],[-.01,.096,.115],[0,.093,.116],[.011,.096,.115],[.029,.091,.111],[0,.089,.116]],lip,0,.08,0,0,11);
      b.tube([-.024,.089,.115],[.025,.089,.115],.0018,.0018,skin.map(c=>c*.35),0,11,6);
    ...
    for (const [sx, bone, fore] of [[0.26, 2, 9], [-0.26, 3, 10]]) {
      ...
      b.loft([ar(sleeveEnd,.061,.066),ar(-.22,.066,.073),ar(-.13,.077,.080),ar(-.045,.085,.087,sx*.94),ar(.015,.072,.071,sx*.90),ar(.042,.037,.043,sx*.87)],top,cloth,bone,{capStart:true,capEnd:true});
    ...
    // Soft baked creases add volume without random noise or a second character draw call.
    for(let i=0;i<b.v.length;i+=13) {
      if(b.v[i+11]!==cloth || !cloth)continue;
      const y=b.v[i+1],bone=b.v[i+12],nx=Math.abs(b.v[i+3]);
      const shade=(bone===0||bone===6)?(.92+.08*Math.max(0,b.v[i+4])):(.94+.06*nx);
      for(let k=0;k<3;k++)b.v[i+6+k]*=shade;
      b.v[i+9]=Math.atan2(b.v[i],b.v[i+2])*.4;b.v[i+10]=y*1.6;
    }
    ...
    for(let v=0;v<b.n;v++) {
      ...
      if(bone===0 && y>.16) {other=6;weight=M.clamp((y-.16)/.32,0,.5);}
      else if(bone===6) {other=0;weight=M.clamp((.48-y)/.32,0,.5);}
      else if (bone===2 || bone===3) {
        if(y>-.15) {other=6;weight=M.clamp((y+.15)/.2,0,.65);offsetY=.6;}
