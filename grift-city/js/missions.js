...
const MISSIONS = (() => {
 ...
 if (reason) HUD.notify(reason + '  (Y to retry)'); if (S.current) { S.retry = { m: S.current, t: 45 }; S.fails[S.current.name] = (S.fails[S.current.name] || 0) + 1; } cleanup(); S.current = null; S.cooldown = 3; }
  ...
  const LIST = [
    ...
    { id: 0, name: 'HOT CAR', auto: true, gentle: true,
      ...
      start(d) { const p = P();
        // It opens in the Foundry Quarter: outside the Lantern Diner, with the car idling at the mouth of Lantern Lane, a
    ...
    { id: 2, name: 'SPECIAL DELIVERY', checkpoints: [dd => { const g0 = laneSpot(6, 6, 0, 1, 20, 0); dd.van = spawnCar('van', g0.x, g0.z, g0.angle, { color: 2, health: 900 }); dd.phase = 1; dd.ambush = true; S.timer = 110; P().x = dd.van.x - dd.van.right[0] * 2; P().z = dd.van.z - dd.van.right[1] * 2; const k = place('docks'); blip(k.x, k.z); objective('Deliver the crate to Pier 9 in Southport.'); }],
      ...
      update(d, dt) { if (d.van.wrecked || d.van.health < d.van.maxHealth * 0.35) return fail('The crate is ruined.'); if (d.phase === 0) return; if (S.timer <= 0) return fail('Too slow. The buyer walked.');
        ...
        if (d.phase === 1 && dist < 260 && !d.ambush) { d.ambush = true; checkpoint(0); for (let i = 0; i < 2; i++) { const s = laneSpot(6 + i, 7, 0, 1, 20, i); const c = spawnCar('muscle', s.x, s.z, s.angle, { mode: 'chase', color: 10 }); c.ai.mode = 'chase'; const p = PEDS.spawn(c.x, c.z, { look: PEDS.GANG, gang: true, hostile: true, weapon: 'uzi', important: true }); p.inCar = c; c.driver = p; S.spawned.push(p); } HUD.notify("Crane's crew are on you!"); }
    ...
    { id: 5, name: 'MIDNIGHT RUN', checkpoints: [dd => { dd.started = true; dd.count = 0; dd.lap = dd.laps - 1; dd.cp = 0; const st = dd.route[dd.route.length - 1]; PLAYER.exitCar(); dd.car.x = st[0]; dd.car.z = st[1]; dd.car.vx = dd.car.vz = 0; P().x = dd.car.x - dd.car.right[0] * 2; P().z = dd.car.z - dd.car.right[1] * 2; for (const r of dd.rivals) { r.rivalLap = dd.laps - 1; r.rivalCp = 0; r.x = st[0] + (W.rng() - 0.5) * 6; r.z = st[1] - 8; r.ai.mode = 'route'; r.ai.routeIdx = 0; } }],
      ...
      update(d, dt) {
        ...
        if (P().car && near(cp[0], cp[1], 8)) { d.cp++; AUDIO.play('checkpoint'); if (d.cp >= d.route.length) { d.cp = 0; d.lap++; if (d.lap >= d.laps) { for (const r of d.rivals) r.ai.mode = 'traffic'; return pass(3000, 'You won the purse. The kids are furious.'); } HUD.big('LAP ' + (d.lap + 1), '#f5c542', 1.5); if (d.lap === d.laps - 1) checkpoint(0); } }
    ...
    // The bank job, in three acts. Case it first if you like: photograph the two door guards and the camera over the
    // door, and you go in knowing the room (fewer guards inside, a shorter drill, a slower police response and a cleaner
    // take). Then walk the crew through the front door into the banking hall, hold the room while they open the vault,
    // and get everyone out and back to the safehouse.
    { id: 7, name: 'THE FIRST GRIFT',
      intro: [['MARLA', 'Bank job. First Grift Bank, Downtown. Two of my people ride with you.'], ['MARLA', "Case it first if you're smart. There's a camera in the car: the two men on the door, and the camera over it. Or don't, and take your chances."], ['MARLA', 'Then walk my crew through the front door, hold the room while they open the vault, and bring everyone back to the safehouse. Everyone.']],
      start(d) { const g = place('mission'); d.crew = []; for (let i = 0; i < 2; i++) { const c = spawnPed(g.x + 3 + i * 2, g.z + 1, { role: 'crew', weapon: 'uzi', health: 160 }); c.weaponOut = true; c.look = PEDS.SWAT; c.mesh = PEDS.getMesh(PEDS.SWAT); d.crew.push(c); } PLAYER.giveWeapon('uzi', 120); PLAYER.giveWeapon('camera', Infinity); if (P().armor < 100) P().armor = 100;
        const b = place('bank'); d.subjects = []; for (const k of [-1, 1]) { const q = spawnPed(b.x + k * 3.2, b.z + 1.2, { name: 'GUARD', stationary: true, weapon: 'pistol', health: 90 }); q.look = PEDS.COP; q.mesh = PEDS.getMesh(PEDS.COP); q.faceTarget = null; q.angle = Math.PI; d.subjects.push({ x: q.x, z: q.z, y: 1.3, ped: q, name: 'a door guard', done: false }); }
        if (b.cam) d.subjects.push({ x: b.cam.x, z: b.cam.z, y: b.cam.y, name: 'the camera', done: false });
        d.cased = 0; d.phase = 0; blip(b.x, b.z); objective('Case First Grift Bank (optional): photograph the door guards and the camera, 0/' + d.subjects.length + '. Or take the crew straight in.'); },
      onPhoto(d, fx, fz) { if (d.phase !== 0) return HUD.notify('No time for pictures now.'); const p = P(); let got = null;
        for (const s of d.subjects) { if (s.done) continue; const dx = s.x - p.x, dz = s.z - p.z, dist = Math.hypot(dx, dz); if (dist < 45 && (dx * fx + dz * fz) / (dist || 1) > 0.9 && W.los(p.x, p.z, s.x, s.z)) { got = s; break; } }
        if (!got) return HUD.notify('Nothing useful in that shot.'); got.done = true; d.cased++; AUDIO.play('checkpoint'); HUD.notify('Got ' + got.name + '. (' + d.cased + '/' + d.subjects.length + ')');
        if (d.cased === d.subjects.length) { HUD.notify('Cased. You know the room, the guard rota and the camera sweep.'); PLAYER.P.stats.cased = (PLAYER.P.stats.cased || 0) + 1; } },
      update(d, dt) { const b = place('bank'); const alive = d.crew.filter(c => c.alive); const room = CITY.interiors.bank; const cased = d.cased >= d.subjects.length;
        if (alive.length !== d.crew.length) return fail('A crew member died. Marla needs everyone back.');
        if (d.phase === 0) { if (!P().car && near(b.x, b.z, 4) && alive.every(c => !c.inCar && M.dist(c.x, c.z, b.x, b.z) < 12)) {
            // through the front door: the hall under the lot takes over, the crew come too and take the vault door
            d.phase = 1; for (const q of d.subjects) if (q.ped && q.ped.alive) { q.ped.hostile = true; q.ped.stationary = false; }
            if (room) { enterInterior(room); for (const [i, c] of alive.entries()) { const [sx, sz] = room.spots.crew[i]; c.x = sx; c.z = sz; c.y = room.floorY; c.guardSpot = { x: sx, z: sz }; }
              d.inside = []; const n = cased ? 1 : 3; for (let i = 0; i < n; i++) { const [gx, gz] = room.spots.guards[i]; const q = PEDS.spawn(gx, gz, { name: 'GUARD', gang: true, hostile: true, weapon: 'pistol', health: 90, important: true }); q.look = PEDS.COP; q.mesh = PEDS.getMesh(PEDS.COP); S.spawned.push(q); d.inside.push(q); } }
            else for (const [i, c] of alive.entries()) c.guardSpot = { x: b.x + (i ? 2.2 : -2.2), z: b.z + 1.6 };
            S.timer = cased ? 35 : 55; POLICE.setStars(cased ? 2 : 3); objective(cased ? 'Hold the hall while the crew opens the vault. You knew where the guard would be.' : 'Hold the hall while the crew opens the vault!'); }
          else if (P().car && near(b.x, b.z, 30)) objective('Park, get out and walk the crew up to the front door.');
          else if (P().car && alive.some(c => !c.inCar)) objective('Wait for the crew to get in.');
          else if (!P().car && near(b.x, b.z, 30)) objective('Walk the crew up to the front door of First Grift Bank' + (d.cased ? ' (cased ' + d.cased + '/' + d.subjects.length + ')' : '') + '.'); }
        else if (d.phase === 1) { objective('The crew is on the vault.  ' + fmt(S.timer)); if (!cased && S.timer < 25 && P().wanted < 4) POLICE.setStars(4);
          if (S.timer <= 0) { d.phase = 2; S.timer = -1; if (S.inside === 'bank') exitInterior(); for (const [i, c] of alive.entries()) { c.guardSpot = null; c.x = P().x + (i ? 1.5 : -1.5); c.z = P().z + 1.5; c.y = P().y; } if (cased) PLAYER.addMoney(2500, 'a clean take'); POLICE.setStars(cased ? 3 : 4); const s = place('safehouse'); blip(s.x, s.z); objective('Vault open, bags full. Get the crew back to the safehouse!'); } }
        else if (d.phase === 2) { const s = place('safehouse'); if (near(s.x, s.z, 8) && alive.every(c => M.dist(c.x, c.z, s.x, s.z) < 14 || (P().car && c.inCar === P().car))) { for (const c of alive) if (c.inCar) c.exitCar(); POLICE.clear(); delete P().weapons.camera; if (P().weapon === 'camera') { P().weapon = 'fist'; P().weaponOut = false; } pass(10000, 'Marla: "Everyone. Good."'); } else if (P().car && alive.some(c => !c.inCar)) objective('Wait for the crew to get in the car.'); else objective('Get the crew back to the safehouse!'); } } },
    { id: 8, name: 'CRANE',
  ...
  function update(dt) {
    ...
    if (S.retry) { S.retry.t -= dt; const key = typeof TOUCH !== 'undefined' && TOUCH.active ? 'Tap here' : 'Press Y'; const fromCp = S.cp && S.cp.name === S.retry.m.name ? ' from the checkpoint.' : '.';
      if (S.current) S.retry = null; else if (p.alive && INPUT.hit('KeyY')) { retry(); return; } else if (S.retry.t <= 0) { S.retry = null; objective(''); } /* a failed job can be retried for a while, then the offer lapses */
      else if (!S.side) objective(S.retry.resumed ? 'Where you left off: ' + key + ' to resume ' + S.retry.m.name + fromCp : 'Mission failed. ' + key + ' to retry ' + S.retry.m.name + fromCp); }
    updateSide(dt); updateShops(dt);
  }
  function start(m) { S.objective = ''; S.current = m; m.data = {}; S.blip = null; S.blips.length = 0; S.markers.length = 0; HUD.big(m.name, '#f5c542', 3); const tries = S.fails[m.name] || 0; S.timerScale = tries >= 2 ? 1.4 : 1; if (tries >= 2) { P().armor = Math.max(P().armor, 50); HUD.notify('Third time lucky: more time on the clock, and a vest.'); } const focus = m.strand === 2 ? S.givers.okafor : m.strand === 'phone' ? null : (m.auto ? null : S.givers.marla); let intro = typeof m.intro === 'function' ? m.intro() : m.intro; if (m.strand === 2 && S.progress > 8 && intro) intro = [['OKAFOR', 'Crane is dead. His lieutenants are not, and they still run his yards and his books.'], ...intro]; /* older saves finished Marla's finale before Okafor's jobs */ if (intro && !S.skipIntro) say(intro, () => { m.start(m.data); }, focus); else m.start(m.data); S.skipIntro = false; if (S.cp && S.cp.name !== m.name) S.cp = null; }
  // A checkpoint inside a long mission: a retry after this point restarts here instead of at the giver.
  // Checkpoints are indexes into the job's own table of restore functions, so a save can name one and a reload can
  // find it again (a closure made mid-update could not be written down).
  function checkpoint(idx) { const m = S.current; if (!m || !m.checkpoints || !m.checkpoints[idx]) return; S.cp = { name: m.name, idx, restore: m.checkpoints[idx] }; HUD.notify('Checkpoint.'); }
