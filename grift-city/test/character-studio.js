...
let studioReady=false, studioView='front', castLook='PLAYER_LOOK', studioActor=null, studioPaused=false, studioYaw=0, studioNight=false;
...
function studioDraw() {
 ...
 const p=PLAYER.P;p.look=PEDS[castLook];p.mesh=PEDS.getMesh(p.look);p.speed=studioView==='walk'?1.5:studioView==='run'?6.8:0;p.aim=studioView==='aim'?1:0;p.crouch=studioView==='crouch'?1:0;p.airborne=studioView==='vault';p.vaultPose=studioView==='vault'?.8:0;p.reloadDuration=1.4;p.reloadT=studioView==='reload'?1.4-(W.state.elapsed%1.4):0;p.state='foot';p.vx=0;p.vz=p.speed;W.state.time=studioNight?22:10.5;p.weapon=['aim','reload'].includes(studioView)?'pistol':'fist';p.weaponOut=['aim','reload'].includes(studioView);p.camPitch=0;p.gesturePulse=studioView==='chat'?(.65+Math.sin(W.state.elapsed*2)*.6):0;
...
let studioTime=0;function studioFrame(now){requestAnimationFrame(studioFrame);if(!studioSetup())return;const dt=Math.min(.04,(now-studioTime)/1000)||0;studioTime=now;if(!studioPaused){W.state.elapsed+=dt;PLAYER.P.phase+=dt*Math.PI*2*PEDS.cadence(PLAYER.P.speed);studioDraw();}}requestAnimationFrame(studioFrame);
