...
let studioImported=new URLSearchParams(location.search).get('character')==='quaternius';
const studioHero={current:{...PEDS.PLAYER_LOOK,importedHero:false},imported:{...PEDS.PLAYER_LOOK,importedHero:true}};
...
function studioDraw() {
 ...
 const p=PLAYER.P;p.look=castLook==='PLAYER_LOOK'?studioHero[studioImported?'imported':'current']:PEDS[castLook];p.mesh=PEDS.getMesh(p.look);p.speed=studioView==='walk'?1.5:studioView==='run'?6.8:0;p.aim=studioView==='aim'?1:0;p.crouch=studioView==='crouch'?1:0;p.airborne=['vault','jump'].includes(studioView);p.vy=studioView==='jump'?4:0;p.landing=studioView==='land'?.8:0;p.vaultPose=studioView==='vault'?.8:0;p.reloadDuration=1.4;p.reloadT=studioView==='reload'?1.4-(W.state.elapsed%1.4):0;p.state='foot';p.vx=0;p.vz=p.speed;W.state.time=studioNight?22:10.5;p.weapon=['aim','reload'].includes(studioView)?studioWeapon:'fist';p.weaponOut=['aim','reload'].includes(studioView);p.camPitch=0;p.gesturePulse=studioView==='chat'?(.65+Math.sin(W.state.elapsed*2)*.6):0;
 ...
 RENDER.setTimeOfDay(W.state.time,0,0);
 window.__renderOnce();studioStatus.textContent=(castLook==='PLAYER_LOOK'?(studioImported?'Imported hero':'Current hero'):castLook)+' · '+studioView+' · '+(studioNight?'night':'day')+' · sound '+(AUDIO.muted?'muted':'on');
}
document.querySelectorAll('[data-model]').forEach(b=>b.onclick=()=>{studioImported=b.dataset.model==='imported';castLook='PLAYER_LOOK';studioDraw();});
