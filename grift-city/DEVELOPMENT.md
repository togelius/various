...
- [x] Escape: preserve held momentum after vaults, add a short jump buffer and ledge grace period, blend the aim camera, preserve camera direction on vehicle exit, and accept early throttle during committed entry. Seated drivers react to steering and impacts. Distant pedestrians and occupants switch to their simpler meshes at 28 m.

Character-art validation: all 12 dependency-free suites, release build and whitespace checks pass; 84 connected-joint poses, 24 planted-foot checks and 56 weighted-skin poses. Inspected front/rear views and named cast in the live muted studio.

Animation validation: 33 new checks cover 30/60/120 Hz transitions, off-hand reach, sideways/backward ground contact and NPC wall contact. All 13 node suites and all 10 browser suites pass (visual, persistence, vehicles, entry, docks, repo, reverse, missions, six-interval soak, pursuit). Browser launch is explicitly muted. The full gate now uses Python timeouts so it works on macOS without GNU coreutils. The elevated-target regression checks barrel direction and relative hand rise, which remain meaningful with a bent elbow.

Escape validation: 19 dedicated simulation checks cover 30/60/120 Hz jump and vault transitions, camera continuity, early throttle and outfit distinctions. The visible `test/escape-rehearsal.html` fixture uses ordinary game input after setup, with a two-star pursuit, traffic, damage and no saves. Its ten-minute endurance run completed steal/collision/abandon/vault/switch, lost the pursuit, and continued through normal vehicle wear: 2.6 km, two driven cars after the first abandonment, 75 health and no respawns. This is a simulation continuity check, not a human completion time or rendering benchmark. The rebuilt character studio remains available at `test/character-studio.html`; it is always muted and does not save.
