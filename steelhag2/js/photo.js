...
'use strict';
const Photo = (() => {
  const W = 640, H = 360;let serial=0;
  ...
  function shoot(scene, chapter, px, pz, vantage) {
    ...
    const k=(vantage?vantage.key:key(chapter,px,pz))+':'+Date.now()+':'+serial++;
    const caption = vantage ? vantage.caption : '';
    const entry = Store.keepPhoto(k, chapter, px, caption, seen, print, vantage&&vantage.key);
