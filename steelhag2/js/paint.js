...
'use strict';
const MAT = { SNOW: 0, ICE: 1, STEEL: 2, RED: 3, PLANK: 4, CONCRETE: 5, BIRCH: 6, ROOF: 7, BEIGE: 8, DARK: 9, GLASS: 10, ROAD: 11, FOIL: 12, REED: 13, FLAT: 14, LED: 15, CABLE: 16, SPRUCE: 17 };
const MATS = 18, TEX = 256;
...
const Paint = (() => {
  ...
  function paintAll() {
    ...
    put(MAT.ICE, (g, r) => {
      ...
      // cracks
      wrapped(g, () => { for (let i = 0; i < 14; i++) { let x = r() * TEX, y = r() * TEX; g.strokeStyle = css('#dfe6ea', 0.55); g.lineWidth = 1 + r(); g.beginPath(); g.moveTo(x, y); for (let k = 0; k < 8; k++) { x += (r() - 0.5) * 40; y += (r() - 0.5) * 40; g.lineTo(x, y); } g.stroke(); } });
    ...
    put(MAT.BIRCH, (g, r) => {
      ...
      wrapped(g, () => { for (let i = 0; i < 70; i++) Art.line(g, r() * TEX, r() * TEX, r() * TEX * 0.2 + r() * TEX, r() * TEX, 1 + r() * 3, '#2b2a26', 0.85); });
