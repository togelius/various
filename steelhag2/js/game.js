...
const Game = (() => {
  ...
  const isTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || (reviewParams.has('review')&&reviewParams.has('touch'));
  ...
  addEventListener('blur',()=>{for(const k in keys)keys[k]=false;lookDX=lookDY=0;if(state==='play'&&!menuStack&&!new URLSearchParams(location.search).has('review'))openMenu('pause');});
  ...
  if (stick) {
    ...
    stick.addEventListener('pointermove', e => { if (e.pointerId !== sid) return; const dx = clamp((e.clientX - cx) / 55, -1, 1), dy = clamp((e.clientY - cy) / 55, -1, 1); touchAxes.x = dx; touchAxes.y = -dy; knob.style.left = 50 + dx * 32 + '%'; knob.style.top = 50 + dy * 32 + '%'; });
    const end = e => { if (e.pointerId !== sid) return; sid = null; touchAxes.x = touchAxes.y = 0; knob.style.left = '50%'; knob.style.top = '50%'; };
  ...
  function pollPad() {
    ...
    const st = { use: p.buttons[0]?.pressed, cut: p.buttons[7]?.pressed || p.buttons[2]?.pressed, torch: p.buttons[3]?.pressed, album:p.buttons[8]?.pressed,camera: p.buttons[1]?.pressed || p.buttons[5]?.pressed, pause: p.buttons[9]?.pressed, hurry: p.buttons[10]?.pressed || p.buttons[6]?.pressed };
  ...
  const sentry = new Machine('bearer',4,33,Math.PI,{aggressive:true,scale:1.45});machines.push(sentry);
  ...
  const sparks={pts:[],data:new Float32Array(96*5),n:0,col:[.72,.88,1]};
  function emitCut(x,y,z){for(let i=0;i<36;i++)sparks.pts.push({x,y,z,vx:(Math.random()-.5)*4,vy:1+Math.random()*3,vz:(Math.random()-.5)*4,life:.25+Math.random()*.45});if(sparks.pts.length>96)sparks.pts.splice(0,sparks.pts.length-96);}
  const snow = { data: new Float32Array(850 * 5), n: 850, col: [0.95, 0.96, 0.98], pts: [] };
  for (let i = 0; i < snow.n; i++) snow.pts.push({ x: (Math.random() - 0.5) * 60, y: Math.random() * 25, z: (Math.random() - 0.5) * 60, s: 0.025 + Math.random() * 0.06, p: Math.random() * TAU });

  // ---------------------------------------------------------------- state
  let endingT=0,wakeFadeT=0,checkpointToast=0,albumPage=0,albumZoom=-1,albumCompare=false,albumRects=[];
  let albumReturn = 'title', islandTransition = 0, health=3, hurtT=0, cutterCooldown=0, cutFlash=0, cutPoint=[0,0,0],cutCandidates=[],combatNotice=null,impactT=0,darkroomRects=[];
  let state = 'boot', stateT = 0, time = 0, chapter = 1, fade = 0, fadeTarget = 1, flash = 0, deathT = 0, deathKind='machine';
  ...
  function applyRig(dt) { rigT = Math.min(1, rigT + dt / 6); const m = mixRig(rigFrom, rig, smooth(0, 1, rigT)); Object.assign(env, m); env.shadowOn = env.sunCol[0] > 0.1;env.iceGlow=flags.hulls?Math.sin(Math.PI*world.hulls[0].rise)*.45:0; }
  ...
  function resetMachine(m,x,z,yaw){const sound=m.onSound;Object.assign(m,new Machine(m.kind,x,z,yaw,{mesh:m.mesh,aggressive:m.aggressive,hidden:m.hidden,scale:m.s/(m.kind==='bearer'?1:.42)}));m.onSound=sound;}
  function checkpoint(){
  ...
  function updatePlay(dt) {
    ...
    const photoRequested = input.usePressed && !P.sit && P.carried<=0 && (P.viewfinder || (!standoffWith && !input.cutHeld && vantage && !nearRelay && !nearestSeat() && Math.hypot(P.x-world.door.x,P.z-world.door.z)>2.3 && Math.hypot(P.x-world.cellar.x,P.z-world.cellar.z)>2.4));
    ...
    if(standoffWith&&P.hold>.05&&P.speed<.08&&!P.cutterUp){const facing=Math.atan2(standoffWith.x-P.x,standoffWith.z-P.z);P.yaw+=M.angleTo(P.yaw,facing)*(1-Math.pow(.001,dt));}
    // the kid
    Tracks.update(P);
    kid.pose(P.x, P.y, P.z, P.yaw, P.speed / 1.4, dt, clamp(P.hold * 2, 0, 1) || (standoffWith ? 0.25 : 0), 0, P.torch, P.cutterUp,[P.vx,P.vz],characterFeedback());
    P.handWorld = kid.hand;
    // machines
    const mctx = { calmCeiling: quiet() ? 1 : calmCeiling, awake: awake && !P.sit, quiet:quiet(), onImpact:m=>{impactT=3.5;M.trs(impact.model,m.x,World.groundY(m.x,m.z)+.012,m.z,0);emitCut(m.x,.08,m.z);}, onHit: m=>{if(hurtT>0)return;health--;hurtT=1.5;combatNotice={text:health===1?'CRITICAL · ONE HIT LEFT':'HIT · KEEP MOVING',t:1.4};Sound.fall();if(health<=0)die('machine');}, onLift: m => { if (quiet()) { m.state = 'wait'; return; } flags.carried = true; carriedFrom = { x: P.x, z: P.z }; P.carried = 0.001; Sound.at('lift', m.x, m.y + 1, m.z); }, onCarried: m => wake(m), onSwitchedOff: m => { if(m===sentry){flags.roadkeeper=true;combatNotice={text:'ROADKEEPER DISABLED',t:3.5};Sound.touch();checkpoint();return;} flags.off = true; lineFor('off'); Sound.touch(); sleepReady = true;checkpoint(); } };
  ...
  function characterFeedback(){
    const age=1.5-hurtT,hit=hurtT>0?smooth(0,.06,age)*(1-smooth(.06,.60,age)):0;
    const c=RENDER.cam,aimPitch=Math.atan2(c.y-c.ty,Math.hypot(c.tx-c.x,c.tz-c.z));
    return {hit:reduce()?0:hit,recoil:reduce()?0:cutFlash/.18,aimPitch,condition:health};
  }
  function fireMiss(){
  ...
  function updateInterior(dt){
    ...
    kid.pose(P.x,P.y,P.z,P.yaw,P.speed/1.4,dt,0,0,P.torch,P.cutterUp,[P.vx,P.vz],characterFeedback());P.handWorld=kid.hand;
  ...
  function openDarkroom() { darkroom=Darkroom.open(Store.photos);darkroomRects=[];state='darkroom';stateT=0;Sound.setIndoor(true);P.viewfinder=false;lineCur=null;lineQ=[];Sound.develop();if(document.exitPointerLock)document.exitPointerLock(); }
  function updateDarkroom(dt) {
    const d=darkroom,hit=tap&&darkroomRects.find(r=>tap.x>=r.x&&tap.x<=r.x+r.w&&tap.y>=r.y&&tap.y<=r.y+r.h);
    if(nav.back||input.pausePressed||hit&&hit.act==='leave'){Sound.setIndoor(Interior.contains(P.x,P.z));state='play';stateT=1;return;}
    const wasReady=d.ready,developed=Darkroom.advance(d,dt,stateT>.2&&(input.usePressed||nav.ok||hit&&hit.act==='reveal'));
    if(developed.length){Store.developBatch(developed);Sound.tick_menu(1);}
    if(!wasReady||stateT<.2)return;
    if(pressed.album||hit&&hit.act==='album'){albumReturn='darkroom';albumZoom=-1;albumCompare=false;state='album';stateT=0;return;}
    const dir=(nav.next||hit&&hit.act==='next'?1:0)-(nav.prev||hit&&hit.act==='prev'?1:0);if(dir)Darkroom.move(d,dir);
    if(input.cameraPressed||input.usePressed||nav.ok||hit&&hit.act==='compare')d.compare=!d.compare;
    if(hit&&Number.isInteger(hit.index)){d.i=hit.index;d.compare=false;}
  }
  ...
  function updateSnow(dt) {
    sparks.n=0;sparks.pts=sparks.pts.filter(p=>p.life>0);for(const p of sparks.pts){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.z+=p.vz*dt;p.vy-=5*dt;sparks.data.set([p.x,p.y,p.z,.05,Math.min(1,p.life*3)],sparks.n++*5);}
    ...
    for (let i = 0; i < lim; i++) { const f = snow.pts[i]; f.p += dt; f.y -= (0.6 + f.s * 4) * dt; f.x += Math.sin(f.p * 0.8) * 0.3 * dt - 0.5 * dt; if (f.y < -2) { f.y = 24; f.x = (Math.random() - 0.5) * 60; f.z = (Math.random() - 0.5) * 60; } if (Math.abs(f.x) < 2.5 && Math.abs(f.z) < 2.5) f.x += 3; d[n * 5] = c.x + f.x; d[n * 5 + 1] = c.y + f.y - 8; d[n * 5 + 2] = c.z + f.z; d[n * 5 + 3] = f.s * 1.5; d[n * 5 + 4] = 0.42; n++; }
    snow.count = n;
  }
  ...
  // Functional instructions use a fixed physical size and a quiet dark backing.
  // The larger serif type remains the voice of the story, not the controls.
  function uiLabel(label,x,y,size=13,col='#e0e6df',align='left'){
    g.font=`${size}px ui-monospace, SFMono-Regular, Menlo, monospace`;g.textAlign=align;g.textBaseline='alphabetic';g.fillStyle=col;g.fillText(label,x,y);
  }
  function uiBox(x,y,w,h,fill='rgba(13,23,27,.88)',stroke='rgba(174,195,190,.20)'){
    g.fillStyle=fill;g.beginPath();g.roundRect(x,y,w,h,5);g.fill();if(stroke){g.strokeStyle=stroke;g.lineWidth=1;g.stroke();}
  }
  function controlHints(title=false){
    if(isTouch)return;
    const k=S().keys,key=a=>keyName(k[a]).toUpperCase(),move=[k.forward,k.left,k.back,k.right].map(keyName).join('').toUpperCase();
    const hints=title?[["ENTER","Select"],["↑ ↓","Choose"],["ESC","Back"]]:P.viewfinder?[[key('use'),'Shutter'],[key('camera'),'Lower camera'],['TAB','Album']]:P.cutterUp?[[move,'Evade'],[key('use'),'Fire'],[key('cut'),'Release to lower']]:[[move,'Move'],['SHIFT','Run'],[key('cut'),'Hold to aim'],[key('camera'),'Camera'],['TAB','Album']];
    if(!title)hints.unshift([locked?'MOUSE':'CLICK',locked?'Look':'Look around']);
    const font=Math.min(16,12*T()),keyFont=Math.min(16,11*T()),gap=16,maxW=UW-28,rows=[[]];let widths=[0];
    for(const [key,label]of hints){g.font=`${keyFont}px ui-monospace, SFMono-Regular, Menlo, monospace`;const kw=Math.max(25,g.measureText(key).width+12);g.font=`${font}px ui-monospace, SFMono-Regular, Menlo, monospace`;const w=kw+7+g.measureText(label).width;
      let i=rows.length-1;if(widths[i]+w+gap>maxW-24&&rows[i].length){rows.push([]);widths.push(0);i++;}rows[i].push({key,label,kw,w});widths[i]+=w+gap;
    }
    const height=rows.length*34+8,y=UH-height-10;
    rows.forEach((row,i)=>{const w=widths[i]-gap+24,x=(UW-w)/2,yy=y+i*34;uiBox(x,yy,w,34);let xx=x+12;for(const h of row){uiBox(xx,yy+6,h.kw,22,'rgba(181,200,190,.10)','rgba(181,200,190,.32)');uiLabel(h.key,xx+h.kw/2,yy+21,keyFont,'#e8dfc9','center');uiLabel(h.label,xx+h.kw+7,yy+21,font);xx+=h.w+gap;}});
  }
  function interactionPrompt(y){
    if(!prompt||menuStack)return 0;
    const captured=P.viewfinder&&Photo.S.polaroid&&Photo.S.polaroid.t<2.8;
    const status=captured||prompt==='relay restored'||prompt===TEXT.prompts.cellarEmpty||prompt===TEXT.prompts.doorLocked||P.cutterUp&&!cutTarget||cutterCooldown>0;
    const label=captured?'PHOTO '+String(Photo.S.polaroid.count).padStart(2,'0')+' · CAPTURED':prompt===TEXT.prompts.hold?'Keep holding':prompt,key=status?'':(P.hold>0?'HOLD ':'')+(isTouch?'USE':keyName(S().keys.use).toUpperCase());
    const size=Math.min(18,14*T()),maxW=Math.min(UW-28,520),lines=[],words=label.split(' ');let line='';
    g.font=`${size}px ui-monospace, SFMono-Regular, Menlo, monospace`;const kw=key?Math.max(30,g.measureText(key).width+14):0,available=maxW-30-(key?kw+12:0);
    for(const word of words){const test=line?line+' '+word:word;if(line&&g.measureText(test).width>available){lines.push(line);line=word;}else line=test;}if(line)lines.push(line);
    const tw=Math.max(...lines.map(l=>g.measureText(l).width)),w=Math.min(maxW,tw+30+(key?kw+12:0)),h=18+lines.length*(size+5),x=(UW-w)/2;
    uiBox(x,y,w,h);g.fillStyle=status?'#819794':'#cdae7d';g.fillRect(x,y+6,2,h-12);
    if(key){uiBox(x+12,y+(h-28)/2,kw,28,'rgba(195,207,195,.13)','rgba(204,218,202,.40)');uiLabel(key,x+12+kw/2,y+h/2+5,size-1,'#f0e5ca','center');}
    lines.forEach((l,i)=>uiLabel(l,x+15+(key?kw+12:0),y+14+size+i*(size+5),size));
    if(P.hold>0){g.fillStyle='#bdd9ca';g.fillRect(x+8,y+h-3,(w-16)*Math.min(1,P.hold/2),2);}
    return h;
  }
  ...
  function renderUI() {
    ...
    if ((state === 'play' || state === 'darkroom')&&deathT<=0) {
    ...
    if(state==='play'&&!menuStack&&!P.viewfinder&&deathT<=0){
      ...
      const goal=inside?'THE HOUSE':chapter===1?(threat?(quiet()?'CALM THE ROADKEEPER':'DISABLE THE ROADKEEPER'):'REACH SJÖGÅRDEN'):sleepReady?'RETURN TO THE FRONT DOOR':standoffWith?'REACH OUT':flags.relay?'APPROACH THE BEARER':'RESTORE THE FIELD RELAY';
      const detail=standoffWith?'Hold '+(isTouch?'Use':keyName(S().keys.use))+' to reach out. Keep still and keep the torch off.':inside?'Listen to the radio and read the notebook, or rest in your old room.':threat?(quiet()?'Lower the cutter, approach and hold still to reach out.':'Aim at the support joints. Step out of the amber charge lane.'):chapter===1?Math.max(0,Math.round(World.SHORE_Z-P.z))+' m to the island · follow the red markers':sleepReady?'Develop your photographs at the side door, or enter the house.':flags.relay?'Lower the cutter and hold still to reach out. Or cut its supports.':'Follow the blue cable behind the barn. The relay is still drawing current.';
      const short=UH<510,compact=UW<760||short,panelX=compact?14/sx:28,panelY=compact?(short?14:68)/sy:90,panelW=compact?Math.min(UW-28,short?390:UW-28)/sx:470;
      ...
      if(compact)promptY=panelY*sy+panelH*sy+10;
    ...
    if(state==='play'&&!menuStack&&deathT<=0){
      const distance=Math.hypot(P.x-sentry.x,P.z-sentry.z),threat=chapter===1&&!sentry.off&&!sentry.dark&&distance<27&&!quiet();
      if(threat||health<3){
        const right=UW-(isTouch?132:28),top=isTouch?34:42;
        for(let i=0;i<3;i++){g.fillStyle=i<health?'#c5d7cd':'#694a43';g.fillRect(right-56+i*21,top,14,5);}
        text('CONDITION',right/sx,(top-8)/sy,11,.85,{align:'right',ui:true});
      }
      const bearing=threat?Combat.indicator([sentry.x,sentry.y+1.4,sentry.z],RENDER.cam,UW/UH):null;
      if(bearing){
        const urgent=sentry.state==='windup'||sentry.state==='lunge',color=urgent?'#ffc58a':'#d8b18a';
        const xx=UW/2+bearing.x*Math.min(500,UW*.38),yy=UH/2+bearing.y*Math.min(190,UH*(UW<760?.10:.31));
        g.save();g.translate(xx,yy);g.rotate(Math.atan2(bearing.x,-bearing.y));g.strokeStyle=color;g.shadowColor='#101619';g.shadowBlur=6;g.lineWidth=2.5;
        g.beginPath();g.moveTo(-7,4);g.lineTo(0,-4);g.lineTo(7,4);g.stroke();g.restore();
        const label=urgent?(bearing.behind?'CHARGE · BEHIND':'CHARGING'):bearing.behind?'BEHIND YOU':'ROADKEEPER',lx=clamp(xx,75,UW-75),ly=yy+(bearing.y>.35?-25:23);
        g.fillStyle='rgba(10,19,23,.82)';g.fillRect(lx-70,ly-13,140,35);
    ...
    if (pl && state === 'play'&&deathT<=0) {
      const inT=smooth(0,.5,pl.t),outT=smooth(5.2,6,pl.t),w=UW<760?132:230,h=w*9/16;
      const x=UW-w-24+outT*(w+40),y=UH-(isTouch?318:242)+(1-inT)*(h+90);
      g.save();g.translate(x+w/2,y+h/2);g.rotate(.035-outT*.15);
      g.fillStyle='rgba(0,0,0,.28)';g.fillRect(-w/2-7,-h/2-5,w+18,h+40);g.fillStyle='#eee9de';g.fillRect(-w/2-8,-h/2-8,w+16,h+38);g.drawImage(pl.canvas,-w/2,-h/2,w,h);
      uiLabel('PHOTO '+String(pl.count).padStart(2,'0')+' · '+(pl.saved?'SAVED':'SESSION'),0,h/2+19,UW<760?10:11,'#41453e','center');g.restore();
    }
    if(state==='play'&&!menuStack&&deathT<=0)controlHints();
    ...
    if(deathT>0){text(deathKind==='ice'?'THE ICE GAVE WAY':'THE ROADKEEPER CAUGHT YOU',640,340,17,.95,{ui:true,col:'#d7d7cd'});text('Returning to the last checkpoint',640,373,15,.75,{italic:true});}
  ...
  function renderTitle() {
    ...
    text(tagline,640,(UH-(isTouch?36:82))/sy,isTouch?13:15,b*.85,{italic:true});
  }
  ...
  function renderDarkroom() {
    const d=darkroom,p=d.list[d.i],compact=UW<760,short=UH<510;darkroomRects=[];
    // A red safelight over a scarred bench, enamel tray and contact strip.
    g.fillStyle='#15110f';g.fillRect(0,0,UW,UH);const glow=g.createRadialGradient(UW*.16,0,10,UW*.16,0,UW*.95);glow.addColorStop(0,'rgba(173,45,22,.42)');glow.addColorStop(.45,'rgba(81,35,24,.24)');glow.addColorStop(1,'rgba(9,12,13,.9)');g.fillStyle=glow;g.fillRect(0,0,UW,UH);
    g.strokeStyle='rgba(173,135,99,.045)';g.lineWidth=1;for(let y=8;y<UH;y+=9){g.beginPath();g.moveTo(0,y);g.lineTo(UW,y+Math.sin(y)*4);g.stroke();}g.strokeStyle='rgba(0,0,0,.35)';for(let y=90;y<UH;y+=110){g.beginPath();g.moveTo(0,y);g.lineTo(UW,y+2);g.stroke();}
    const label=(s,x,y,size=14,opt={})=>text(s,x/sx,y/sy,size,1,{align:'left',ui:true,col:'#dfc7a7',...opt});
    const button=(title,act,x,y,w,index)=>{g.fillStyle='rgba(217,185,146,.09)';g.fillRect(x,y,w,38);g.strokeStyle='rgba(217,185,146,.26)';g.strokeRect(x+.5,y+.5,w-1,37);label(title,x+w/2,y+24,12,{align:'center'});darkroomRects.push({act,index,x:x/sx,y:y/sy,w:w/sx,h:38/sy});};
    label('SJÖGÅRDEN / BASEMENT',24,short?25:35,compact?10:12,{col:'#bb8564'});label('THE DARKROOM',24,short?49:65,compact?18:23,{letter:2});
    label(d.ready?'PRINT '+String(d.i+1).padStart(2,'0')+' / '+d.list.length:'DEVELOPING '+d.pending.length+' EXPOSURE'+(d.pending.length===1?'':'S'),UW-24,short?26:40,compact?10:12,{align:'right',col:'#bb8564'});
    const top=short?78:105,bottom=short?UH-82:UH-178,available=Math.max(70,bottom-top),w=Math.min(UW-(compact?48:190),Math.max(120,(available-48)*16/9),800),h=w*9/16,x=(UW-w)/2,y=top+(available-h-48)/2;
    g.shadowColor='rgba(0,0,0,.55)';g.shadowBlur=24;g.shadowOffsetY=12;g.fillStyle='#424a46';g.fillRect(x-20,y-20,w+40,h+88);g.shadowBlur=0;g.shadowOffsetY=0;
    g.strokeStyle='#788078';g.lineWidth=2;g.strokeRect(x-19,y-19,w+38,h+86);g.fillStyle='#1d2929';g.fillRect(x-10,y-10,w+20,h+64);
    g.fillStyle='#ddd4bd';g.fillRect(x-4,y-4,w+8,h+52);g.fillStyle='#c5bca5';g.fillRect(x,y,w,h);
    const readyImage=img=>img&&(!(img instanceof Image)||img.complete&&img.naturalWidth),u=Darkroom.reveal(d),img=p&&(d.compare?p.img:p.print);
    if(readyImage(img)){g.globalAlpha=d.compare?1:smooth(0,1,u);g.drawImage(img,x,y,w,h);g.globalAlpha=1;}
    if(!d.ready){g.fillStyle='rgba(141,46,21,'+(.45*(1-u))+')';g.fillRect(x,y,w,h);g.fillStyle='#b7764f';g.fillRect(x,y+h+40,w*u,3);}
    const caption=p&&p.caption||'An unmarked frame.';const lines=wrap(caption,(w-12)/sx,compact?13:16);lines.slice(0,2).forEach((line,i)=>label(line,UW/2,y+h+19+i*17,compact?13:16,{align:'center',ui:false,col:'#393c35',italic:true,shadow:false}));
    label(d.compare?'ORIGINAL EXPOSURE':d.ready?'DEVELOPED PRINT':'IMAGE EMERGING',UW/2,y-33,10,{align:'center',col:'#c99c79'});
    if(!short){const count=Math.min(compact?3:7,d.list.length),start=clamp(d.i-Math.floor(count/2),0,Math.max(0,d.list.length-count)),tw=compact?76:88,th=tw*9/16,gap=12,xx=(UW-(count*tw+(count-1)*gap))/2,yy=UH-120;
      for(let i=0;i<count;i++){const photo=d.list[start+i],tx=xx+i*(tw+gap);g.fillStyle=start+i===d.i?'#d2b68e':'#544a3d';g.fillRect(tx-3,yy-3,tw+6,th+6);const im=d.ready?photo.print:photo.img;if(readyImage(im))g.drawImage(im,tx,yy,tw,th);darkroomRects.push({index:start+i,x:(tx-3)/sx,y:(yy-3)/sy,w:(tw+6)/sx,h:(th+6)/sy});}
    }
    const bw=compact?[38,38,106,64,64]:[50,50,190,130,110],gap=compact?6:10,total=bw.reduce((a,b)=>a+b,0)+gap*4,xx=(UW-total)/2,yy=UH-49;
    let bx=xx;[['←','prev'],['→','next'],[!d.ready?'Reveal now':d.compare?'Show print':'Compare',!d.ready?'reveal':'compare'],['Album','album'],['Leave','leave']].forEach(([title,act],i)=>{button(title,act,bx,yy,bw[i]);bx+=bw[i]+gap;});
    if(!compact&&!short)label(d.ready?'← → browse   ·   C / E compare   ·   Tab album   ·   Esc leave':'The whole roll develops together. Press E to reveal it now.',UW/2,UH-145,12,{align:'center',col:'#aa8c70'});
  }
  const albumSize=()=>UW<760?1:6;
  function updateAlbum(){
    if(stateT<.2)return;
    const pages=Math.max(1,Math.ceil(Store.photos.length/albumSize()));albumPage=clamp(albumPage,0,pages-1);
    ...
    if(hit&&hit.act==='download'&&albumZoom>=0){const p=Store.photos[albumZoom],src=p.developed&&!albumCompare?p.neg:p.src;if(src&&src.startsWith('data:image/')){const a=document.createElement('a');a.href=src;a.download='stalhagen-photo-'+(albumZoom+1)+'.jpg';a.click();}return;}
    ...
    else{albumPage=clamp(albumPage+dir,0,pages-1);if(hit&&Number.isInteger(hit.index)){albumZoom=hit.index;albumCompare=false;}else if(nav.ok&&Store.photos.length)albumZoom=albumPage*albumSize();}
  }
  function renderAlbum(){
    if(UW<760)return renderCompactAlbum();
    g.fillStyle='#161c20';g.fillRect(0,0,UW,UH);albumRects=[];const a=Math.min(1,stateT/.3),list=Store.photos,n=list.length,pages=Math.max(1,Math.ceil(n/albumSize()));
    ...
    if(albumZoom>=0&&list[albumZoom]){
      ...
      if(p.developed)button(albumCompare?'Show print':'Compare','compare',550,680,180);button('Save image','download',960,680,160);
      text((albumZoom+1)+' / '+n,1238,680,14,.7,{align:'right',ui:true});return;
    }
    const start=albumPage*albumSize();
    list.slice(start,start+albumSize()).forEach((p,i)=>{const x=110+i%3*365,y=111+Math.floor(i/3)*246,w=330,h=186,img=p.developed?p.print:p.img;
  ...
  function renderCompactAlbum(){
    g.fillStyle='#161c20';g.fillRect(0,0,UW,UH);albumRects=[];const list=Store.photos,n=list.length,index=albumZoom>=0?albumZoom:albumPage,p=list[index];
    const label=(t,x,y,size=14,opt={})=>text(t,x/sx,y/sy,size,1,{ui:true,...opt});
    const button=(t,act,x,y,w)=>{g.fillStyle='rgba(210,210,190,.1)';g.fillRect(x,y,w,40);label(t,x+w/2,y+25,14);albumRects.push({act,x:x/sx,y:y/sy,w:w/sx,h:40/sy});};
    label('THE FIELD ALBUM',20,34,14,{align:'left'});label(n?(index+1)+' / '+n:'0 photographs',UW-20,59,12,{align:'right',col:'#a9b4b5'});
    if(p){const isPrint=p.developed&&!albumCompare,img=isPrint?p.print:p.img,w=UW-40,h=w*9/16,x=20,y=Math.max(100,(UH-h-105)/2);
      g.fillStyle='#e7e1d4';g.fillRect(x-5,y-5,w+10,h+62);if(img&&(!(img instanceof Image)||img.complete&&img.naturalWidth))g.drawImage(img,x,y,w,h);
      const lines=wrap(p.caption||'Unmarked frame',(w-12)/sx,14);lines.slice(0,2).forEach((l,i)=>text(l,UW/2/sx,(y+h+24+i*17)/sy,14,1,{italic:true,col:'#343b39',shadow:false}));
      label(isPrint?'DEVELOPED PRINT':'ORIGINAL EXPOSURE',UW/2,y-18,11,{col:'#a9b4b5'});
      if(albumZoom<0){albumRects.push({index,x:x/sx,y:y/sy,w:w/sx,h:(h+56)/sy});label('Tap the photograph to inspect it',UW/2,y+h+85,11,{col:'#a9b4b5'});}
      else{button('Save image','download',UW-145,75,125);if(p.developed)button(albumCompare?'Show print':'Compare','compare',UW/2-70,y+h+78,140);}
    }else label('No photographs yet.',UW/2,UH/2,18);
    const bw=(UW-60)/3,y=UH-56;button('←','prev',20,y,bw);button(albumZoom>=0?'Album':'Return','back',30+bw,y,bw);button('→','next',40+bw*2,y,bw);
  }

  // ---------------------------------------------------------------- loop
  let last = performance.now(),reviewFrozen=false;
  function frame(now) {
    let dt = Math.min(0.12, (now - last) / 1000); last = now; tuneResolution(dt);
    const n = Math.ceil(dt / (1 / 60)); if(!reviewFrozen)for (let i = 0; i < n; i++) update(dt / n);
